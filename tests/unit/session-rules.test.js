...
describe("session-scoped CHZZK redirect rules", () => {
  it("builds least-privilege DNR rules scoped to a single CHZZK tab", () => {
    const rule = buildScopedSessionRule({ policy, tabId: 42 });
  ...
  it("fails closed unless a lower-quality HLS request comes from a CHZZK live tab", () => {
    ...
    assert.deepEqual(shouldBootstrapSessionRule(eligible, policy), {
      ok: true,
      quality: "720p",
      reason: "eligible-lower-quality-chzzk-hls",
      tabId: 7,
    });
