(() => {
  ...
  var quality_policy_default = {
    ...
    notes: [
      ...
      "A trusted HLS master response is streamed to CHZZK unchanged, bounded by probeMaxBytes for local parsing, accepted only with an exact successful response URL/status, and scored before the response filter closes so its highest eligible variant is cached before the first numeric rendition request. Master-response observers are released on filter stop/error, terminal completion/error, ineligible redirect, generic-CDN or destructive context migration, and tab closure. An eligible dedicated-livecloud observer is re-keyed under the new token across a same-tab, same-stream mini-player transition, while an eligible master redirect chain also retains it. Candidate URLs must pass the request-domain and quality-marker policy before ranking by resolution, frame rate, then bitrate; one malformed or untrusted high-ranked entry cannot hide a lower valid variant, and a valid advertised quality is not capped to the numeric fallback grid. A bounded no-credential fetch is used only when response filtering or redirect lifecycle observation cannot attach.",
