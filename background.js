(() => {
  ...
  var quality_policy_default = {
    ...
    notes: [
      ...
      "The isolated site-observer content script runs at document_start on CHZZK live pages and only sends a live-page-ready message. A separate MAIN-world player-controller uses a query-compatible /live* manifest match plus an exact runtime pathname guard for /live and /lives, with no extension API or media-URL access; it selects and persists only the highest concrete track accepted by CHZZK's own quality filter.",
      ...
      "Without a cached target, eligible configured candidates start as one concurrent batch within probeResolutionBudgetMs so timed-out synthetic tiers cannot starve a usable lower candidate; results are consumed from highest to lowest, and only concurrent requests in the same playlist family share that in-flight resolution.",
      ...
      "Request URL, initiator, method, resource type, trusted request domain, and CHZZK context constrain redirects; explicit foreign metadata vetoes cache, and a same-site non-live CHZZK document may continue small-player playback only on the legacy dedicated CHZZK livecloud host suffixes or exact livecloud.akamaized.net URLs with an exact chzzk path segment. A URL-only live-to-list/search or later mini-player SPA transition re-keys a trusted master-derived or verified dedicated-host target, one whose numeric response verifier actually attached and remains pending/valid, an eligible dedicated master observer whose current or pending redirect URL remains dedicated, or the single unambiguous still-running dedicated-host candidate scan per playlist family under the new context token. Matching response verifiers remain ordered under the migrated target rather than being classified by an unobservable player owner: a newer mini-player request supersedes an older failure, while a genuine failure from the first mini-player request remains authoritative even when it started before tabs.onUpdated. Unattached/failed numeric verification, generic-CDN work and observers, dedicated observers awaiting a generic redirect, aborted work, and conflicting source contexts are discarded. Firefox's stale original live documentUrl cannot re-adopt the old context or authorize a generic CDN, including during URL-less reload validation. Full document loads, new live-page state, foreign navigation, and tab close invalidate the target and pending work. Origin-only CHZZK metadata also requires a dedicated host unless the tab was authoritatively prewarmed as live; metadata-free contextless compatibility is limited to that exact dedicated set rather than generic CDN path markers.",
      "Prewarm marks the CHZZK live tab only; it is a supporting signal, not the sole gate. Install/startup and content-message prewarming re-read the current tab under an unchanged per-tab transition token before migrating reusable verified contextless target/response state or an eligible origin-bound dedicated master observer into an authoritatively confirmed live context, so delayed snapshots cannot erase a target or master response established by an earlier playlist request or overwrite newer mini-player/navigation state. The runtime resolves the best actually available HLS variant from trusted playlist evidence instead of seeding a fixed startup quality.",
  ...
  // src/shared/ad-request-policy.js
  var CHZZK_AD_WEB_REQUEST_URLS = Object.freeze([
  ...
  var CANCELLED_AD_PATHS = [
    /^\/ad-polling\/v1\/lives\/[^/]+\/ad\/?$/,
    /^\/service\/v1\/lives\/[^/]+\/ads\/current\/?$/,
    /^\/service\/v1\/seoraksan\/?$/,
  ];
  var LIVE_DETAIL_PATH = /^\/service\/[^/]+\/channels\/[^/]+\/live-detail\/?$/;
  var VIDEO_DETAIL_PATH = /^\/service\/[^/]+\/videos\/[^/]+\/?$/;
  ...
  function chzzkAdRequestDecision(details) {
  ...
  function playlistNameDiscriminator(segment) {
    ...
    stem = stem.replace(/([_-])((?:ll)?hls)(?:[_-]+)(?:master|playlist|chunklist)(?=$|[_-])/gi, "$1$2");
    stem = stem.replace(/([_-])(?:master|playlist|chunklist)(?=$|[_-])/gi, "");
    stem = stem.replace(
      /^(?:chunklist(?:_\{quality\})?|(?:ll)?hls(?:[_-]+(?:master|playlist))*|index|manifest|master|media|playlist)(?:[_-]+|$)/i,
      "",
    );
    stem = stem.replace(/([_-])\{quality\}([_-])/gi, "$1");
  ...
  var HLS_PLAYLIST_EXTENSION_CASE_VARIANTS = ["M3U8", "M3u8", "m3U8", "m3u8"];
  ...
  function contextRequiresDedicatedHls(evidence, tabId, trustedLiveTabIds, miniPlayerTabIds2) {
    return (
      miniPlayerTabIds2?.has?.(tabId) ||
  ...
  function isTrustedChzzkContext(
    details,
    policy,
    { miniPlayerTabIds: miniPlayerTabIds2 = null, trustedLiveTabIds = null } = {},
  ) {
    if (!details || !isValidRedirectTabId(details.tabId)) return false;
    const evidence = requestContextEvidence(details, policy);
    if (evidence.veto) return false;
    if (contextRequiresDedicatedHls(evidence, details.tabId, trustedLiveTabIds, miniPlayerTabIds2)) {
  ...
  function isTrustedMasterPlaylistRequest(
    details,
    policy,
    { miniPlayerTabIds: miniPlayerTabIds2 = null, trustedLiveTabIds = null } = {},
  ) {
    ...
    if (evidence.veto) return false;
    if (evidence.trusted) {
      return contextRequiresDedicatedHls(evidence, details.tabId, trustedLiveTabIds, miniPlayerTabIds2)
  ...
  function createPlaylistProbe({
    ...
    const probeMaxBytes2 = positiveNumber(policy.probeMaxBytes, 256e3);
    const probeTimeoutMs = positiveNumber(policy.probeTimeoutMs, 1500);
    function urlQualityMarkersMatch2(url, expectedQuality) {
      const qualities = parseQualitiesFromUrl(url);
      return (
    ...
    function playlistEvidenceSupportsExpectedQuality2(evidence, expectedQuality) {
      ...
      if (variants.length > 0 || /#EXT-X-STREAM-INF:/i.test(evidence.text)) {
        return variants.some((variant) => {
          ...
          return (
            ...
            urlQualityMarkersMatch2(variant.url, expectedQuality)
          );
        });
      }
  ...
  function createSessionStateStore({
    ...
    redirectedRequestsById: redirectedRequestsById2 = /* @__PURE__ */ new Map(),
  } = {}) {
    ...
    if (!(redirectedRequestsById2 instanceof Map)) {
    ...
    function forgetRedirectedRequests(sessionKey) {
      for (const [requestId, record] of redirectedRequestsById2) {
        if (record.key !== sessionKey) continue;
        record.settled = true;
        redirectedRequestsById2.delete(requestId);
      }
    }
    ...
    function sweepExpired(now = Date.now()) {
      ...
      for (const [key, state] of activeTargetsBySession2) {
        if (state.expiresAt == null || state.expiresAt > now) continue;
        activeTargetsBySession2.delete(key);
        forgetRedirectedRequests(key);
        removedActiveTarget = true;
      }
      for (const [key, state] of failedTargetsBySession2) {
        if (!(state.targets instanceof Map)) {
          failedTargetsBySession2.delete(key);
          continue;
        }
      ...
      for (const [key, state] of resolutionBySession2) {
        if (state.controller?.signal?.aborted) resolutionBySession2.delete(key);
      }
      return removedActiveTarget;
    }
    ...
    return Object.freeze({
      activeTargetsBySession: activeTargetsBySession2,
      enforceLimits,
  ...
  var {
    ...
    maxStates: quality_policy_default.maxSessionStates,
    maxStatesPerTab: quality_policy_default.maxSessionStatesPerTab,
  ...
  function playlistSession(details) {
    ...
    return {
      contextKey,
      dedicatedHls: isDedicatedChzzkHlsPlaylistUrl(details.url, quality_policy_default),
      familyKey,
  ...
  function attachMasterResponseObserver(details) {
    ...
    if (existing && !existing.settled) {
      ...
      const expectedRedirectMismatch =
        existing.expectedRedirectNetworkUrl && existing.expectedRedirectNetworkUrl !== finalNetworkUrl;
  ...
  function handleMasterResponseRedirect(details) {
    ...
    if (
      !redirectNetworkUrl ||
      !isTrustedMasterPlaylistRequest(redirectDetails, quality_policy_default, redirectOptions)
    ) {
      settleMasterResponseObserver(record);
      return;
    }
  ...
  function masterResponseObserverCanMigrateAcrossContext(record) {
    const currentOrPendingNetworkUrl = record?.expectedRedirectNetworkUrl ?? record?.finalNetworkUrl;
    return Boolean(
      record &&
      !record.settled &&
      record.session.dedicatedHls &&
      record.streamFailed !== true &&
      isDedicatedChzzkHlsPlaylistUrl(currentOrPendingNetworkUrl, quality_policy_default),
    );
  }
  ...
  function migrateTabQualityState(
    ...
    const tabResolutions = [...resolutionBySession.entries()].filter(([, state]) => state.tabId === tabId);
    const tabTargets = [...activeTargetsBySession.entries()].filter(([, state]) => state.tabId === tabId);
    const resolutionGroups = /* @__PURE__ */ new Map();
    const targetGroups = /* @__PURE__ */ new Map();
    const preservedTargetByOldKey = /* @__PURE__ */ new Map();
    ...
    for (const group of resolutionGroups.values()) {
      if (group.length !== 1) {
        for (const { state: state2 } of group) state2.controller.abort();
        continue;
      }
      const [{ key, state }] = group;
  ...
  async function prewarmLiveTab(tabId, url = null, { migrateVerifiedContextless = false } = {}) {
  ...
  function handleTrustedPlaylistRequest(details, attachedRedirectVerifier, inheritedBudget = null) {
    ...
    const decision = shouldRedirectRequest(details, quality_policy_default, redirectOptions);
    if (decision.ok) {
      try {
        const session = playlistSession(details);
        const targetState = session ? activeTargetForSession(session) : null;
  ...
  function handleRequest(details) {
    ...
    if (hasTrustedChzzkMetadata(details, quality_policy_default)) {
      if (
        !miniPlayerTabIds.has(details.tabId) &&
        isChzzkLiveUrl(details.documentUrl, quality_policy_default)
      ) {
