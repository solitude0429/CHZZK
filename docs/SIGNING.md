...
- full current head SHA를 포함하고 `@codex review`로 시작하는 위 operator request 뒤에 reviewer가 관찰된 두 clean issue-comment 형식 중 하나를 남김: 표준 `Codex Review: Didn't find any major issues.` + `Reviewed commit` 10~40자리 hex ref, 또는 `## Review Result` + `No major issues found in exact head` + 40자리 full SHA. 어느 형식이든 ref는 repository commit API에서 현재 full head SHA로 유일하게 해석됨

두 번째 경로에서는 reaction timestamp가 GitHub가 기록한 현재 PR activity timestamp(`updated_at`), request comment update, 그리고 같은 exact head의 마지막 `COMMENTED`/`CHANGES_REQUESTED` finding review보다 늦어야 합니다. 세 번째 경로는 reviewer login만 신뢰하지 않습니다. comment의 actor type이 `Bot`이고 `performed_via_github_app`의 ID/slug가 reviewer login에서 파생해 GitHub `/apps/{slug}`에서 다시 조회한 exact App과 일치해야 합니다. full-SHA operator request와 clean comment는 모두 편집되지 않았고, clean comment는 request보다 ID와 timestamp가 모두 늦으며, 전체 issue comment 목록에서 가장 최신이고, creation timestamp가 최종 재조회한 PR `updated_at`과 같아야 합니다. 같은 operator의 후속 bare trigger는 허용하지만, full-SHA request와 clean reply 사이에 다른 actor가 새 `@codex review`를 작성하거나 더 오래된 comment를 그 command로 편집하면 binding은 실패하며 operator가 새 unedited full-SHA request를 작성해야 합니다. abbreviated ref는 current head prefix여야 하며 `/repos/{repository}/commits/{ref}`가 반환한 full SHA도 current head와 같아야 합니다. 같은 시각이나 그 뒤에 exact-head finding review가 있으면 clean comment를 인정하지 않습니다.

따라서 아직 PR head가 아니던 commit SHA에 미리 받은 reaction/comment나 이전 clean signal을 뒤의 finding에 재사용할 수 없습니다. `COMMENTED`와 `CHANGES_REQUESTED` 자체는 completion evidence가 아닙니다. PR 자체에 달린 issue-level `+1`도 commit SHA를 담지 않으므로 인정하지 않습니다. identity, App provenance, SHA resolution, state, ordering, 또는 필요한 timestamp가 없거나 malformed이면 gate는 실패합니다.

workflow는 `pull_request_target`, review/review-comment, issue-comment event에서 trusted default branch만 checkout하며 PR code를 실행하지 않습니다. comments/reactions, reviews, review threads와 PR metadata를 두 번 수집해 actor, App, body/update, reaction, review/thread ID/state, head/base/labels/activity snapshot이 같을 때만 두 번째 snapshot을 평가합니다. 따라서 whole-second timestamp 안에 끼어든 새 comment/finding도 latest-comment ID만 stale하게 남긴 채 통과할 수 없습니다. reaction 전용 Actions event가 없으므로 PR `opened`/`synchronize`에서는 bounded polling하고, 15분 schedule은 open non-draft PR마다 동일 workflow의 quiet reconciliation dispatch를 보냅니다. `force_review=true` 수동 run은 run-ID가 포함된 고유 non-cancelable concurrency group에서 generation-bound failure check를 게시하고, 같은 PR의 기존 ordinary review runs만 취소해 종료를 확인한 뒤 failure marker를 다시 게시합니다. 다른 force generation과 그 follow-up은 이 취소 대상에서 제외됩니다. 그 다음 `security-review-required` label을 durable하게 기록하고 같은 generation을 가진 forced reevaluation을 명시적으로 dispatch하므로 label API visibility가 늦어도 review 요구가 유지됩니다. 이 follow-up도 고유 non-cancelable group에서 끝까지 실행됩니다. 다른 status run은 current failure check의 `external_id`와 generation이 일치하지 않으면 cached 결과를 게시할 수 없습니다. check 선택은 monotonic check-run ID를 기준으로 하고, 상태 변경이 필요할 때에는 POST 직전에 check state를 다시 읽습니다. 일반 evaluate/status run은 새 event나 reconciliation이 오면 이전 일반 run을 취소합니다. Reconciliation은 evidence 결과가 바뀐 경우에만 exact-head `CHZZK review completion` check를 새로 게시하므로 reaction/comment 생성·삭제 후 stale success/failure가 자동으로 교정됩니다. 강제 gate를 해제하려면 label을 명시적으로 제거합니다. 어느 review entry point에도 release dispatch 권한은 없습니다.
