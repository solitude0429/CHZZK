...
- full current head SHA를 포함하고 `@codex review`로 시작하는 위 operator request 뒤에 reviewer가 표준 `Codex Review: Didn't find any major issues.` issue comment를 남기고, 그 comment의 `Reviewed commit` 10~40자리 hex ref가 repository commit API에서 현재 full head SHA로 유일하게 해석됨
