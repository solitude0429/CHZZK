...
- URL-marker-only media evidence is restricted to its playlist family and a `markerEvidenceTtlMs` idle timeout. Redirected response chunks are passed to the player immediately and copied only up to `probeMaxBytes`; renewal requires both a successful 2xx completion and an exact streamed body that passes HLS/quality validation. An exact-URL HTTP 304 may instead renew prior validated target evidence after an empty cache revalidation, independent of body/status event order. Status alone, empty/HTML/malformed or oversized non-304 bodies, other redirects, browser-visible errors, and HTTP 204/205 or 4xx/5xx completions invalidate and temporarily suppress the failed target so re-resolution can downgrade without looping.
