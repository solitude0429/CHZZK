...
- The exact-head review gate classifies broad sensitive paths plus explicit release/security labels and identifies the reviewer by exact login. It accepts either a non-dismissed review whose `commit_id` equals the current head or a reviewer `+1` strictly newer than that head's committer timestamp, while requiring zero unresolved threads. A configured operator's full-SHA issue-comment reaction is preferred; the observed issue-level fallback has timestamp-only binding and its documented non-monotonic-timestamp limitation. Repository protection source-binds `CHZZK review completion` to GitHub Actions, preserves other required checks, requires conversation resolution, applies to administrators, and intentionally configures no approving-review/last-push rule that a sole PR author cannot satisfy.
