...
- The final exact diff is reviewed through one unedited PR-author or configured-operator request created after the current head entered the PR. The request binds the full head SHA to the exact base ref/SHA, and only a Codex-connector thumbs-up on that same comment is accepted while its body and the head/base remain unchanged. Timeline, reaction, and thread pagination fail closed; native conversation resolution remains required. High-risk release, permission, deployment, and security-policy impact is called out in the PR body before the request. The sole-owner branch does not require an impossible self-approval or a duplicate bot-review check.
