...
- Redirect handling is constrained by tab, CHZZK context, trusted request domains, request methods, resource types, and an actual `.m3u8` pathname. The blocking listener uses case-complete playlist-only URL patterns, so media segments bypass it and query-only playlist strings remain ineligible. Explicit non-CHZZK document/origin metadata always vetoes cached trust. A same-site non-live CHZZK page can continue small-player playback only on the two dedicated livecloud host suffixes and cannot authorize a generic CDN playlist. A URL-only live-to-list/search SPA route change migrates only a verified dedicated-host target or one whose response verifier actually attached and has not failed into authoritative mini-player state; stale original-live `documentUrl` metadata cannot re-adopt the old context, including during URL-less reload validation. Unresolved probes, unattached/failed verification, generic-CDN targets, collisions, expired evidence, failed-target suppression, full document loads, concrete live-page trust, and all foreign-navigation state are removed.
...
- Probe bodies require `#EXTM3U` as the exact first meaningful line plus a usable master variant, media segment, or LL-HLS part reference; they reject obvious HTML/JSON MIME types and are capped in UTF-8 bytes. HLS attributes reject duplicate keys and accept only bounded positive decimal bandwidth/frame-rate syntax.
...
- The exact-head review gate classifies both current and previous renamed sensitive paths, all `docs/**`, `README.md`, and explicit release/security labels and identifies the reviewer by exact login. It accepts an exact-head approval, or the trusted reviewer GitHub App's latest unedited standard clean-review comment only when its entire known template follows an unedited full-SHA operator `@codex review` command and matches the current head/activity; same-second issue comments are ordered by ID. Unrelated SHA mentions, mixed/trailing clean-response content, and unbound PR-level reactions remain invalid. A manual `force_review=true` request runs in a unique non-cancelable group, publishes a generation-bound failure check, cancels and awaits older ordinary same-PR review runs, persists `security-review-required`, and explicitly dispatches the matching forced reevaluation. Review/comment events re-evaluate immediately; a comment-bound reaction-only approval requires one explicit manual reevaluation. There is no recurring repository-wide reconciliation workflow.
