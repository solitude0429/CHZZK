...
- Probe bodies require `#EXTM3U` as the exact first meaningful line plus a usable master variant, media segment, or LL-HLS part reference; an `#EXT-X-PART` marked `GAP=YES` is unavailable and cannot by itself establish or renew quality evidence. Obvious HTML/JSON MIME types are rejected, bodies are capped in UTF-8 bytes, and HLS attributes reject duplicate keys and accept only bounded positive decimal bandwidth/frame-rate syntax.
...
- The final exact diff is reviewed through one unedited PR-author request created after the current head entered the PR while the PR remains draft. The request binds the full head SHA to the exact base ref/SHA. A trusted-base workflow serializes PR-state invalidation against active polling, requires zero unresolved threads, and converts the matching Codex-connector reaction ID/time into durable check-run evidence bound to the request-comment ID, head, and base. Request edits/deletion, source or base transitions, and PR title/body edits fail closed; protected `main` disables force-pushes and deletion. Later review feedback that does not alter the diff is enforced by native conversation resolution, while any source/base/metadata change requires a fresh request. The sole-owner branch does not require an impossible self-approval or a duplicate bot-review check.
