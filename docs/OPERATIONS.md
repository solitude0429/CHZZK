...
3. `npm run verify` 통과
4. `unzip -l dist/chzzk-<version>.zip`로 runtime 파일만 포함되는지 확인
5. GitHub PR 생성 후 CI 통과 확인
6. merge 후 tag/release 생성
7. AMO unlisted signing이 필요하면 `docs/SIGNING.md` 절차 수행
...
3. `npm run diagnostics:analyze -- diagnostics.json` 실행
4. URL shape 변경이면 test-first로 fixture 추가
5. `src/shared/quality.js` 또는 `src/shared/session-rules.js` 수정
6. `npm run verify` 통과 후 PR
...
2. `shouldRecordDiagnostics` 테스트 추가
3. `src/shared/session-rules.js`에서 context gate 강화
4. `npm run verify`
5. 새 release 생성 전 기존 release note에 privacy caveat 추가
