...
5. Open a GitHub PR and wait for all required CI/review gates. CI must pass full verification on both the exact PR head and GitHub's effective pull-request event tree, plus the real Firefox functional E2E. For release/security paths or the explicit review labels, `CHZZK review completion` requires zero unresolved threads plus either the configured reviewer's exact-head `APPROVED` review or its canonical, unedited clean-result comment after an operator request containing the full current head SHA, with one 10–40 hex reviewed-commit prefix matching the current head. The clean comment must be the latest reviewer comment, contain no non-allowlisted trailing text, equal current PR activity time, and postdate any exact-head finding. Reactions are never completion evidence because their deletion has no subscribed Actions event. The gate collects head/activity/labels, comments, reviews, and thread state twice in opposite orders and compares the canonical evidence itself, including same-second changes. The status-publication step reruns that trusted checker immediately before reading or changing the check, preserves pending force markers, replaces labeled markers only for the matching generation or after current label state is reconciled, and uses the validated evaluate head only to publish a fail-closed result when the final lookup cannot recover a head. `COMMENTED`/`CHANGES_REQUESTED` reviews are not completion evidence. Pushes trigger bounded polling, and a quiet 15-minute reconciliation corrects missed events or drift. Release/supply-chain changes should also receive independent human review; record the exception and rationale when that is unavailable.
