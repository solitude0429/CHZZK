...
5. Uses WebDriver's physical window-rectangle command to move a real Firefox window from wide to compact and back to wide. A responsive fixture remounts the player at each actual CSS breakpoint, applies its page-owned `selected` setter asynchronously, and requires exactly one `1080p` selection write and commit per generation. A muted canvas-capture `MediaStream` drives a real `<video>` element throughout the remounts; Firefox must keep advancing `currentTime` and rendered-frame callbacks without a new `waiting`, `stalled`, or `emptied` event after the stable baseline. Separate local synthetic HLS fetches must remain at `1080p` in all three viewport phases without upper-tier probes. This is deterministic Firefox media-element and extension-routing evidence, not a claim that the fixture reproduces CHZZK's proprietary decoder.
