...
The unit suite covers asynchronous player setters, one-shot strong-readiness recovery after early metadata, repeated-evidence negative controls, per-generation and cross-generation write bounds, non-periodic recovery, eligible-route-only stable refill, and waiting/stalled holds across temporary player absence, eligible route changes, and track/media replacement. Player-specific cases cover interrupted-settle waiting history, quality-pane descendant mutation coalescing, bounded silent initial filter/track expansion from both allowed and initially filter-rejected low selections, exact remembered-track compact correction across filter-function and quality-pane replacement, a late second demotion within the five-second physical window, expiry and no-physical negative controls, ignored-compact-write recovery after silent wide expansion, newly allowed higher-track hold enforcement, intent invalidation, and detached-generation storage rejection. It also includes direct library-boundary misuse tests for canonical release basenames, verifier-buffer deployment (no validated-path reread), exact remote draft/tag recovery, immutable deployment checks, bounded lock cleanup, canonical SemVer, administrator dispatch ordering, and idempotent sole-owner repository protection. The repository-settings tests source-bind CI, Firefox E2E, dependency review, and CodeQL to the GitHub Actions app, remove the retired bot-review state, restrict Actions to GitHub-owned code, and keep workflows read-only by default. Workflow-policy tests also require SHA-pinned actions and separated secret/write authority.
...
1. Installs a synthetic controller-less historical `0.1.18` XPI through geckodriver.
...
5. First keeps a live document open across the add-on update, confirms the new controller initially selects the fixture's only allowed `720p`, expands the same filter silently with no DOM, track, or media event, and requires a bounded scan to select and persist `1080p`. It then uses WebDriver's physical window-rectangle command to move a real Firefox window from wide to compact and back to wide. One responsive fixture remounts the player at each actual CSS breakpoint, applies its page-owned `selected` setter asynchronously, and requires exactly one `1080p` request and commit per generation. A fresh-document fixture then keeps the same player, track list, pane, and filter identities while a physical compact transition performs a page-owned `720p` demotion and one bounded waiting/playing cycle. It deliberately ignores one or two fuse-bounded compact `1080p` requests, requires the selected track to remain `720p` without replacing stored `1080p` intent, physically returns to wide, and reallows `1080p` 400 ms later without a DOM, track, or media event. The next bounded scan must issue and commit exactly one post-expansion `1080p` request. A post-recovery resize storm must add no request, ignore, or commit. A muted canvas-capture `MediaStream` drives a real `<video>` element throughout both scenarios; Firefox must keep advancing `currentTime` and rendered-frame callbacks, and neither compact demotion nor the later wide recovery may add `stalled` or `emptied`. Separate local synthetic HLS fetches remain at `1080p` without upper-tier probes. This is deterministic Firefox media-element and extension-routing evidence, not a claim that the fixture reproduces CHZZK's proprietary decoder.
...
11. Serves strict `updates.json` for the current production-manifest version over HTTPS, calls `AddonManager.findUpdates`, and confirms the controller-less historical fixture updates to that version.
