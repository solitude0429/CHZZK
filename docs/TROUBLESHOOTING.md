...
The controller therefore runs a physical-resize scan on the next task after page listeners, coalesces DOM/remount bursts for 250 ms, treats option mutations inside the existing quality pane as fresh evidence, and runs only three bounded follow-up scans at 250 ms, 1 s, and 3 s. A first confirmed sub-1080p generation gets the same bounded discovery window, including when that selected low track is initially above a temporarily contracted filter. This catches a later track or filter expansion even when CHZZK emits no event. Physical resize evidence lasts at most five seconds and is bound to the exact player and track list; every scan resolves the currently mounted quality pane, so removing the old pane and mounting a compact pane does not erase the exact remembered track. Route changes, player/track-list replacement, expiry, and stop remove the evidence. The current pane and filter remain strict identities for each individual write transaction. The controller confirms asynchronous selection through read-only checks and caps an unchanged transaction at two writes. Its cross-generation budget begins with four ordinary writes, followed only by one non-periodic five-second recovery and one final attempt gated by genuine current-primary playback recovery; page-owned remounts cannot reset those bounds, and time spent outside an eligible player route cannot refill them. A never-confirmed first candidate whose setter ignored both ordinary writes can receive one additional local attempt only from `playing`, `canplay`, or `timeupdate` on the exact current primary; early metadata cannot spend it, the global budget is not refilled, and the attempt cannot repeat. On an unchanged primary, a `waiting` hold requires 250 ms of `playing`, `canplay`, or `timeupdate` and is not released by time alone; `stalled` during that settle retains the waiting history. The hold migrates across track-list and primary replacement and remains active while the player is temporarily absent on an eligible route; media events outside eligible routes are ignored. Ordinary selection writes remain held. During an exact physical context, only the previously confirmed concrete track may bypass that hold when CHZZK demotes it through a compact filter; a newly exposed higher track still waits for settle. Without physical evidence, a buffering-only lower selection remains provisional and cannot replace stored high intent until playback settles. A late second compact demotion can be corrected inside the same five-second window, but further writes remain fuse-bounded. If compact writes are ignored, the filter-rejected and reallowed phases are separate transactions, so a silent wide expansion can restore high quality. The Firefox functional test verifies a silent initial `720p`-to-`1080p` promotion, physically resizes a real browser window from 1200 to 560 pixels and back for remounted and same-node players, deliberately keeps the same-node player at `720p` by ignoring compact high-track requests, silently reallows `1080p` 400 ms after returning wide, and requires exactly one post-expansion request and commit. It also checks continuing media time/rendered frames and rejects a post-recovery resize write storm. This deterministic fixture validates the extension boundary, not CHZZK's proprietary decoder.
