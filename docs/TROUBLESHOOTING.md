...
The runtime treats prewarm as a supporting signal only. A trusted master playlist starts non-blocking variant scoring by resolution, frame rate, and bitrate and supersedes an older pending numeric probe only in the same tab/context/playlist family. Without a master, the first numeric request starts one shared candidate resolution per family. Only `.m3u8` patterns enter the blocking listener, media segments bypass it, and an already-cached target redirects synchronously. Trust validation and candidate resolution share one 50 ms request deadline; if resolution is not ready the listener fails open while background probing continues. On a same-site CHZZK list/search page, small-player continuation is limited to the two dedicated livecloud hosts. A URL-only live-to-list/search SPA route change migrates a verified target or matching in-flight response verifier into authoritative mini-player state rather than restarting probes, even when Firefox continues attaching the original live `documentUrl`; generic-CDN targets, unresolved probes, collisions, expired evidence, and full document loads are not preserved. URL-marker-only evidence expires after a 30-second idle `markerEvidenceTtlMs`; a redirected response renews that timeout only after Firefox passes the bytes through unchanged and the bounded incremental decoder proves a usable HLS body. Rewriting and renewal share one safe URL-marker grammar, so the accepted `/360p/.../chunklist_480p.m3u8` legacy form renews normally instead of periodically re-probing. An exact-URL HTTP 304 is the bodyless exception: it renews the previously validated target because Firefox reuses that cached playlist, and empty-body handling waits for status so either event order is safe. Other status-only, bodyless, malformed, redirected, or failed responses cause expiry or temporary downgrade suppression rather than indefinite renewal. Each probe and the whole resolution have time/size limits. Candidate bodies require an exact first meaningful `#EXTM3U` line and are capped in UTF-8 bytes. Live-page changes, foreign navigation, or tab close abort and invalidate the corresponding pending work.
