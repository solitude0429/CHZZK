...
- Release authenticity requires the separate stock-Firefox gate with default signature enforcement, permanent installation, exact ID/version/update URL, and `SIGNEDSTATE_SIGNED`. The release workflow runs install mode on the final AMO-signed XPI before attestation/publication; update mode runs previous-signed to final-signed only after the production update endpoint is deployed.
- Build, AMO-secret signing, signed verification, OIDC attestation, and `contents: write` publication are separate jobs.
- Repository immutable releases must be enabled. An out-of-band administrator preflight binds the exact default-branch SHA/version and dispatches only after `enabled: true`; the workflow has no direct release `workflow_dispatch` and no Administration credential. Before AMO, compatible partial drafts are recognized only after exact commit/tag/name/byte verification. Publication must return `immutable: true`; mismatch or orphan tag states fail closed.
...
- Release/security review gating combines sensitive paths with explicit labels and requires zero unresolved actionable threads plus either the configured connector's non-dismissed exact-`commit_id` review or its `+1` strictly after the head committer timestamp. Full-SHA operator-comment reactions are preferred over the residual timestamp-only issue reaction; missing/malformed identity or time state fails closed.
