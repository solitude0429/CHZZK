...
- Prewarms CHZZK live tabs at `document_start` without choosing a quality, then resolves and caches the best supported quality label per playlist family. Cached redirects return synchronously. Delayed install/startup prewarming migrates verified contextless state into the confirmed live context instead of erasing it. A verified dedicated-livecloud target is migrated across a live-to-search/list mini-player `pushState` without another candidate scan, even when Firefox continues reporting the original live `documentUrl`. A URL-less reload keeps mini-player host restrictions until `tabs.get()` authoritatively validates the current page. Same-URL reload clears quality evidence; navigation and tab-close cancellation tokens prevent stale probes from restoring old state.
