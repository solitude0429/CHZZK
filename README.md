...
- Parses trusted HLS master playlists, including CHZZK's `hls_playlist.m3u8` and `llhls_playlist.m3u8` names, and scores variants by resolution, then frame rate, then bitrate. Master-advertised qualities are not capped to the numeric fallback grid.
...
- Prewarms CHZZK live tabs at `document_start` without choosing a quality, then resolves and caches the best supported quality label per playlist family. Cached redirects return synchronously. A verified target below the configured maximum gets one non-blocking, upward-only refresh per minute, so successful response renewal cannot leave a stream stuck indefinitely at an earlier 720p result when 1080p later becomes provable; failed, equal, or lower refreshes keep the current target. Delayed install/startup and content-message prewarming re-read the current tab under a transition token before migrating verified contextless state into the confirmed live context, so a stale snapshot cannot overwrite newer mini-player state or force another probe. A verified dedicated-livecloud target or its single in-flight candidate scan is re-keyed across live-to-search/list and repeated mini-player `pushState` transitions, even when Firefox continues reporting the original live `documentUrl`; generic-CDN work is never carried into mini-player mode. A URL-less reload keeps mini-player host restrictions until `tabs.get()` authoritatively validates the current page. Same-URL reload clears quality evidence; full navigation and tab-close cancellation tokens prevent stale probes from restoring old state.
