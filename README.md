...
Numeric media-playlist evidence necessarily relies on the requested URL marker because ordinary media bodies do not declare rendition resolution. That evidence is scoped to its secret-free playlist family and uses `markerEvidenceTtlMs` as an idle timeout. Firefox streams each redirected response through unchanged while the background page retains only a bounded copy for HLS validation; only a usable playlist body paired with a successful completion renews the timeout, so status alone cannot keep an empty, HTML, or malformed target alive. An exact-URL HTTP 304 may renew the already validated target because Firefox reuses its cached playlist; empty-body judgment waits for completion status so this cache path is not mistaken for a broken HTTP 200. This avoids periodic blocking probes during valid small-player playback. HTTP 204/205, other redirects, an exposed request error, a 4xx/5xx completion, or invalid streamed evidence invalidates and temporarily suppresses the target so the next request can re-resolve or downgrade without looping.
