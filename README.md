...
- Selects the highest concrete manual track accepted by CHZZK's own player filter after metadata load, delayed track availability, internal track changes, and player remounts, then persists the official `live-player-video-track` `{label,width,height}` value. It does not fake a `480p` label or invent a track that CHZZK did not expose. The player-only controller runs in Firefox's MAIN world because CHZZK rejects synthetic quality-menu clicks. It is loaded across the already-required CHZZK origin so a home-to-live SPA transition cannot miss it, but every selection scan applies an exact `/live` or `/lives` pathname guard and rejects lookalike or departed routes. It contains no extension API, signed URL, or credential access.
