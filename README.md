...
Personal Firefox WebExtension that keeps CHZZK live playback on the highest officially allowed quality.
...
- Selects the highest concrete manual track accepted by CHZZK's own player filter after metadata load, delayed track availability, internal track changes, and player remounts, then persists the official `live-player-video-track` `{label,width,height}` value. Quality-option mutations inside an existing pane count as fresh evidence. A first confirmed track below `1080p`, including a selected low track that the initial filter temporarily rejects, receives only three follow-up scans at 250 ms, 1 s, and 3 s, so a silent initial track/filter expansion is detected without permanent polling; unchanged state causes no write. Physical resize, orientation, and viewport signals create a five-second recovery context bound to the exact player and track list, resolve the currently mounted official quality pane on every scan, run an immediate post-page-listener scan, and then use the same three bounded follow-ups. If CHZZK demotes a previously confirmed high track during a compact-filter contraction, only that exact remembered concrete track may bypass an active media hold and be reasserted under the normal two-write and controller-wide fuses; a late second page demotion and a quality-pane replacement inside the physical window are covered, while a newly allowed higher track still waits for playback to settle. Without physical evidence, a buffering-only lower selection remains provisional and cannot replace the remembered high intent until the exact current primary settles. Route changes and stop cancel the physical context, and remembered-track removal or player/track-list replacement invalidates mismatched intent. The current pane and filter remain strict transaction identities, so an in-flight write cannot commit across their replacement. If compact reassertions are ignored, the filter-rejected and reallowed phases are separate bounded transactions, so a later physical wide transition can recover after a silent filter expansion. Asynchronous page-owned selection setters are confirmed without repeated writes. A never-confirmed initial candidate whose first two writes were ignored may receive exactly one additional local write only after `playing`, `canplay`, or `timeupdate` evidence from the exact current primary media; early metadata, repeated evidence, and unrelated media cannot rearm it, and the shared global budget is never refilled by this recovery. A controller-wide budget starts at four ordinary writes and is not reset by page-owned identity churn. Exhaustion permits one non-periodic five-second recovery; after that, one genuine current-primary unsettled-to-playable transition may open one final bounded attempt. Only confirmed stable selection on an eligible player route replenishes capacity one write per five seconds; time spent on a departed route never earns writes or recovery gates. On an unchanged primary, a `waiting` hold is released only after `playing`, `canplay`, or `timeupdate` remains settled for 250 ms; elapsed time alone cannot release it, and a new `stalled` event inside that window retains the waiting history. The hold survives track-list and primary replacement as well as a temporarily absent player during an eligible-route remount, while media events captured outside an eligible route are ignored. It does not fake a `480p` label or invent a track that CHZZK did not expose. The player-only controller runs in Firefox's MAIN world because CHZZK rejects synthetic quality-menu clicks. It is loaded across the already-required CHZZK origin so a home-to-live SPA transition cannot miss it, but every selection scan applies an exact `/live` or `/lives` pathname guard and rejects lookalike or departed routes. It contains no extension API, signed URL, or credential access.
