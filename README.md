...
- Selects the highest concrete manual track accepted by CHZZK's own player filter after metadata load, delayed track availability, internal track changes, and player remounts, then persists the official `live-player-video-track` `{label,width,height}` value. Responsive resize/remount bursts are coalesced, asynchronous page-owned selection setters are confirmed without repeated writes, and each unchanged player/track generation has a bounded two-write budget that regains only one write per stable five-second interval. A controller-wide budget starts at four ordinary writes and is not reset by page-owned identity churn. Exhaustion permits one non-periodic five-second recovery; after that, one genuine current-primary unsettled-to-playable transition may open one final bounded attempt. Only confirmed stable selection on an eligible player route replenishes capacity one write per five seconds; time spent on a departed route never earns writes or recovery gates. On an unchanged primary, a `waiting` hold is released only after `playing`, `canplay`, or `timeupdate` remains settled for 250 ms; elapsed time alone cannot release it, and a new `stalled` event inside that window retains the waiting history. The hold survives track-list and primary replacement as well as a temporarily absent player during an eligible-route remount, and a replacement or stalled-only primary may additionally clear through one bounded one-second ready-state check for the entire unsettled episode. A temporary responsive filter contraction never makes the extension actively demote or persist over an already selected higher concrete track. It does not fake a `480p` label or invent a track that CHZZK did not expose. The player-only controller runs in Firefox's MAIN world because CHZZK rejects synthetic quality-menu clicks. It is loaded across the already-required CHZZK origin so a home-to-live SPA transition cannot miss it, but every selection scan applies an exact `/live` or `/lives` pathname guard and rejects lookalike or departed routes. It contains no extension API, signed URL, or credential access.
