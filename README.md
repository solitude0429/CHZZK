...
- Prewarms CHZZK live tabs at `document_start` without choosing a quality, then resolves and caches the best supported quality label per playlist family. Cached redirects return synchronously. Delayed install/startup prewarming is non-destructive, so it cannot erase a target that playback established first. A verified dedicated-livecloud target is migrated across a live-to-search/list mini-player `pushState` without another candidate scan, even when Firefox continues reporting the original live `documentUrl`. Same-URL reload clears quality evidence but authoritatively revalidates live-tab trust; navigation and tab-close cancellation tokens prevent stale probes from restoring old state.
