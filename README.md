...
- Falls back to one shared, bounded background probe set per tab, live context, and secret-free playlist family when only a numeric variant playlist URL is available. Independent playlist families never share a target or in-flight promise. The blocking listener waits at most 50 ms and fails open while resolution continues.
